 ...
 * <p>Description: 只需要一个BaseDao就可以，
 * 需要访问数据库的地方，比如BaseService<T>
 *     只需要private BaseDao<T> baseDao即可 </p>
 *
 * @Author 陈德元
 * @data: 2017-05-25
 * @version: 1.0
...
public class BaseDao<T> {
	 ...
	 *              失败会抛出异常
	 */
	public T saveOrUpdate(@NotNull final T model) {
		T merge = entityManager.merge(model);
		return merge;
	}
